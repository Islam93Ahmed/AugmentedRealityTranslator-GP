...
public class BackPropagation extends Thread {
	...
	//private String weightsFile = "Weights.txt";
	...
	public BackPropagation(Context con)
	{
		this.context = con;
	}
	...
	public void set_weights(String file_name)
	{
        Set_data(file_name);
	}
	...
	public void Set_data(String file_name)
	{ 
		try {

            String line,structure;

            BufferedReader reader = new BufferedReader(new InputStreamReader(context.getAssets().open(file_name)));

			int x,y,layer;

			structure = reader.readLine();
            layer=Integer.parseInt(structure);
			NumberOfLayers=layer;
			Layer = new LAYER[layer];
			//structure=bw.readLine();
            structure = reader.readLine();
            x=Integer.parseInt(structure);
			
			Layer[0] = new LAYER(x,x);
			for(int i=1;i<layer;i++)
			{
				//structure=bw.readLine();
                structure = reader.readLine();
                y=Integer.parseInt(structure);
				Layer[i] = new LAYER(y,Layer[i-1].Node.length);
			...
			for(int i=1;i<Layer.length;i++)
			{
				for(int j=0;j<Layer[i].Node.length;j++)
				{
                    ...
                    Layer[i].Node[j].Threshold=Double.parseDouble(line);
					
					for(int k=0;k<Layer[i].Node[j].Weight.length;k++)
					{
						//line=bw.readLine();
                        line = reader.readLine();
                        Layer[i].Node[j].Weight[k]=Double.parseDouble(line);
						//System.out.println(Layer[i].Node[j].Weight[k]);
					}
